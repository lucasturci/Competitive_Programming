...
// pro flow
ed arestas[2 * ((1 << 10) * 12 + 20)];
int cur = 0;
int d[(1 << 10) + 20], ptr[(1 << 10) + 20];
vector<int> edges[(1 << 10) + 20];
 
void add_edge(int u, int v, ll cap) {
	arestas[cur] = ed(v, cap, 0);
	edges[u].pb(cur);
	cur++;
 
	arestas[cur] = ed(u, cap, cap);
	edges[v].pb(cur);
	cur++;
}
...
ll flow(int s, int t) {
	...
	while(bfs(s, t)) {
		...
		while(ll tot = dfs(s, 100000000000000ll, t)) {
			f += tot;
		}
	}
	return f;
...
int main() {
	...
	ll mid;
	ll l = 0, r = 100000000000000ll, x, ans;
    while(l <= r) {
		mid = l + (r - l)/2;
		...
		for(int i = 0; i < (1 << S); ++i) {
			add_edge(s, i + 1, mega[i]);
		}
 
		for(int i = 0; i < (1 << S); ++i) {
			for(int j = 0; j < S; ++j) {
				if((i >> j) & 1) add_edge(i + 1, (1 << S) + j + 1, 100000000000000ll);
			}
		}
 
		for(int j = 0; j < S; ++j) {
			add_edge((1 << S) + j + 1, t, much[j]);
		}
